...
extension NDArray {
    // @differentiable(wrt: self, vjp: _subscriptVJP(array:) where Scalar == Float)
...
internal extension NDArray where Scalar == Float {
    static func _subscriptVJP(array: NDArray<Float>) -> (Int) -> (Float, (Float) -> NDArray<Float>) {
        return { index in
            let value = array[index]

            return (value, { (dvalue: Float) -> NDArray in

                var dself = NDArray(Array(repeating: 0, count: array.nElements), shape: array.shape)
                dself.data[index] = dvalue
